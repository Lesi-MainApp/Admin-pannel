...
const PapersPage = () => {
  ...
  // ✅ Answer count allowed 2..10 (schema rule)
  const answerCounts = useMemo(() => Array.from({ length: 9 }, (_, i) => i + 2), []);
  ...
  const [form, setForm] = useState({
    ...
    timeMinutes: "",
    questionCount: "",
    oneQuestionAnswersCount: 5, // ✅ NEW (Answer count)
    createdPersonName: "",
  ...
  const validate = () => {
    ...
    // ✅ Answer count validate (2..10)
    const ac = Number(form.oneQuestionAnswersCount);
    if (!ac || ac < 2 || ac > 10) e.oneQuestionAnswersCount = "Answer count must be 2..10";
  ...
  const onSubmit = async (ev) => {
    ...
    const payload = {
      ...
      // ✅ send answer count
      oneQuestionAnswersCount: Number(form.oneQuestionAnswersCount || 5),
    ...
    try {
      ...
      dispatch(setLastCreatedPaper(res?.paper || null));
      ...
      setForm((prev) => ({
        ...
        timeMinutes: "",
        questionCount: "",
        oneQuestionAnswersCount: 5, // ✅ reset
        createdPersonName: "",
        payment: "free",
        amount: "",
        attempts: 1,
    ...
    <div className="w-full min-h-screen flex items-center justify-center bg-[#F7F6F6] px-3">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-md border border-gray-200 p-6">
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          {/* Subject OR Stream + Stream Subject */}
          {!isAL ? (
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                Subject
            ...
            <>
              ...
              <div>
                ...
                {errors.streamSubjectId && (
                  <p className="text-xs text-red-600 mt-1">{errors.streamSubjectId}</p>
                )}
              </div>
            </>
          )}
